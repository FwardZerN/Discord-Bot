import { Client, GuildTextBasedChannel, Message, MessageActionRow, MessageEmbed, MessageSelectMenu, MessageSelectMenuOptions, MessageSelectOptionData, SelectMenuInteraction } from "discord.js";
...
export default class Play implements ICommand {
    ...
    public async handle(message: Message, args: string[]): Promise<any> {
        ...
        const option:MessageSelectOptionData[]=searchResult.map((e,i) => ({label:e.uploader.name.slice(0,25),description:e.name.slice(0,50),value:i.toString()}))
        ...
        const selectMsg=await message.channel.send({components:[actionRaw]})
        ...
        try{
            ...
            await selectMsg.edit({
                components:[new MessageActionRow().addComponents(disableSelect)]
            })
